...
|Power Automate Flow-Konto|Ein [aktives Power Automate](https://flow.microsoft.com) für den CRM-Systemadministrator oder Systemanpasser. Dieser Benutzer sollte sich vor [der Power Automate](https://flow.microsoft.com) mindestens einmal bei der Anmeldung anmelden.|
...
Um die Empfehlungen über Partner Center Salesforce CRM hinweg zu synchronisieren, muss die Power Automate-Lösung microsoftspezifische Empfehlungsfelder eindeutig identifizieren. Diese Abgrenzung bietet Partner-Verkäuferteams die Möglichkeit zu entscheiden, welche Empfehlungen sie für den Co-Selling an Microsoft freigeben möchten.
...
2. Aktivieren **Sie Opportunity-Teams,** indem Sie die folgenden Schritte ausführen: 
    - Verwenden Sie unter Setup das Feld **Schnellstart,** um die Teameinstellungen für Verkaufschance zu suchen.
...
>[!NOTE]
>Wenn Sie in einer Sandbox installieren, müssen Sie den anfänglichen Teil der URL durch ersetzen. http://test.salesforce.com

4. Fügen Sie microsoft Solutions in Salesforce der Liste Opportunity related **(Verkaufschance)** hinzu. Wählen Sie nach dem Hinzugefügten das **Schraubenschlüsselsymbol aus,** und aktualisieren Sie die Eigenschaften.

## <a name="best-practice-test-before-you-go-live"></a>Bewährte Methode: Testen, bevor Sie live gehen

Bevor Sie die Lösung installieren, konfigurieren und Power Automate in der Produktionsumgebung anpassen, müssen Sie die Lösung auf einer CRM-Staginginstanz testen.
...
9. Navigieren Sie nach Abschluss der Installation zurück zum Power Automate [und](https://flow.microsoft.com) wählen Sie **im linken** Navigationsbereich Lösungen aus. Beachten **Sie, Partner Center Synchronisierung von** Empfehlungen für Salesforce in der Lösungsliste verfügbar ist.

10. Wählen **Partner Center Synchronisierung von Empfehlungen für Salesforce aus.** Die folgenden Power Automate und Entitäten sind verfügbar:
...
2. Wählen Sie **in der** Dropdownliste Umgebungen in der rechten oberen Ecke die CRM-Instanz aus, in der Sie die Power Automate installiert haben.
...
4. Wählen **Sie in** der linken Navigationsleiste Verbindungen aus, und wählen Sie die Lösung "Partner Center Empfehlungen" aus der Liste aus.
...
- Suchen Sie Partner Center in der Suchleiste in der oberen rechten Ecke nach Empfehlungen (Vorschau).
...
-  Nachdem Sie alle Verbindungen hinzugefügt haben, sollten die folgenden Verbindungen in Ihrer Umgebung zu sehen sein:
...
5. Wählen **Partner Center Salesforce CRM (Insider Preview) aus.**

6. Wählen Sie das **Symbol Bearbeiten** aus, und wählen Sie When a HTTP request is **received (Wenn eine HTTP-Anforderung empfangen wird) aus.**

7. Wählen Sie das **Symbol Kopieren** aus, um die bereitgestellte HTTP POST-URL zu kopieren.

   :::image type="content" source="images/salesforce/copy-url.png" alt-text="Kopier-URL.":::

8. Wählen Sie nun den Flow "Partner Center Webhookregistrierung (Insider-Vorschau)" aus Power Automate und wählen Sie **Ausführen aus.**
...
10. Geben Sie die folgenden Details ein:
    ...
    1. **HTTP-Triggerendpunkt:** URL aus dem früheren Schritt kopiert
...
Der Webhook kann jetzt lauschen, um Ereignisse zu erstellen und zu aktualisieren.

## <a name="customize-synchronization-steps"></a>Anpassen von Synchronisierungsschritten

Wenn Co-Sell-Empfehlungen zwischen Partner Center und Ihrem CRM-System synchronisiert werden, werden die Felder, die auf einem Partner Center PC synchronisiert werden, hier aufgeführt.

CRM-Systeme sind häufig stark angepasst. Sie können die Power Automate anpassen. Befolgen Sie den Leitfaden zur Feldzuordnung, und nehmen Sie bei Bedarf entsprechende Änderungen an den Schritten der Power Automate vor.  Microsoft Partner Center zu CRM-Zuordnungen werden bereitgestellt, aber basierend auf Ihrer CRM-Umgebung können Sie die Felder weiter anpassen.

Mehrere Schritte der einzelnen Power Automate können basierend auf Ihren Anforderungen angepasst werden. Im Folgenden finden Sie Beispiele für verfügbare Anpassungen:

1. So passen Sie die Felder für die Erstellungs- oder Aktualisierungsereignisse in der Partner Center CRM-Empfehlungssynchronisierung an:

   1. Wählen Partner Center Salesforce CRM (Insider Preview) aus.
...
## <a name="end-to-end-bi-directional-co-sell-referral-synchronization"></a>Bidirektionale End-to-End-Co-Sell-Empfehlungssynchronisierung

Nachdem Sie die Power Automate-Lösung installiert, konfiguriert und angepasst haben, können Sie die Synchronisierung von Co-Sell-Empfehlungen zwischen Salesforce CRM und Partner Center.
...
Ein Satz benutzerdefinierter Felder ist als Teil der Entität Partner Center Empfehlungssynchronisierung für Salesforce CRM-Lösung **Opportunity verfügbar.** Ein CRM-Administratorbenutzer muss einen separaten CRM-Abschnitt mit den benutzerdefinierten **Opportunity-Feldern** erstellen.

Die folgenden benutzerdefinierten Felder sollten Teil des CRM-Abschnitts sein:

- **Sync with Partner Center**:Whether to sync the opportunity with Microsoft Partner Center (Synchronisieren mit Microsoft Partner Center)

- **Empfehlungsbezeichner:** Ein schreibgeschütztes Bezeichnerfeld für Microsoft Partner Center-Empfehlung

- **Empfehlungslink:** Ein schreibgeschützter Link zur Empfehlung im Microsoft Partner Center
